...
## Этап 7 — Освещение, прозрачность, расширения (в работе)
...
- **Освещение (Этап B) ✅** — `js/render/Lighting.js`: point (омни, аттенюация) и
  directional (параллельный) источники; flat-shading Ламберт + ambient
  (`Config.AMBIENT`). Rasterizer: мировая нормаль грани + центроид →
  `computeFaceLight` → глиф из `GlyphMap.RAMP` по яркости, цвет = материал × свет
  (per-cell). Источники в `scene.lights` (round-trip toJSON/load). Проверено:
  scene_test 68 / render_test 110 ✓.
- **Текстуры (Этап C) ✅** — `js/render/Texture.js`: именованный реестр ascii-текстур
  (`brick`, `checker`, `window`, `grass`, `water`), `sample` (bilinear + clamp),
  генерация UV на углах граней. Rasterizer: `u/w`,`v/w`,`1/w` → перспективно-
  корректные `u,v` → символ текстуры; цвет = материал × свет. Привязка: `"texture"` /
  `"textures"`. Проверено: scene_test 83 / render_test 145 ✓.
- **Интеграция, сцены, документация (Этап D) ✅** — встроенные сцены (`city_block`,
...
**Приёмка (освещение + текстуры, Этапы B–D):** свет создаёт градиент символов на
гранях (видны освещённые и теневые стороны); текстура отображается на грани;
сцены с освещением/текстурами рендерятся без ошибок консоли, save/load сохраняет
`lights` и материал. Осталось: прозрачные объекты корректно перекрываются.
...
| 7 — Освещение, прозрачность, расширения | 🔄 в работе (освещение + текстуры ✅) |
