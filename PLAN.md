...
10. **HUD** (FPS, позиция, подсказки) + горячие клавиши добавления объектов.
11. **Оптимизации** (back-face, frustum) + полировка + clamp dt.
12. **Scene.toJSON()** + «Save scene» (Blob download) + round-trip проверка.
13. **Сцены** `desert`, `character_demo` — заполняем оставшиеся из структуры.
14. **Документация** (`README.md`: управление, архитектура) + заготовки
    под освещение/прозрачность.
