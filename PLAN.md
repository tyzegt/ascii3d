...
> сохранение/загрузка сцены, frustum culling, документация). **Этап 7** в работе:
> освещение и текстуры реализованы и интегрированы в сцены (см. `FEATURES_PLAN.md`);
> осталось — прозрачность, кватернионы, LOD, пикер объектов (раздел 7 ниже).
...
- **Прозрачность**: в `GlyphMap` — символы с «прозрачностью» (пробел, `·`, `:`).
  ...
  back-to-front. (Позже.)
- **Освещение** ✅: `js/render/Lighting.js` — point (омни, аттенюация по
  расстоянию) и directional (параллельный) источники; flat-shading Ламберт +
  ambient (`Config.AMBIENT`). Rasterizer берёт мировую нормаль грани + центроид →
  `Lighting.computeFaceLight` → глиф из `GlyphMap.RAMP` по яркости, цвет =
...
15. ✅ **Освещение** — `js/render/Lighting.js`: point/directional источники,
    Ламберт + ambient, per-face свет → глиф из `GlyphMap.RAMP` + per-cell цвет.
    Интегрировано в сцены (city_block, desert, character_demo).
